...
class Solution:
    def minKBitFlips(self, nums: List[int], k: int) -> int:
        ...
        n = len(nums)
        totalFlipCount = currFlipCount = 0
        isFlippedAt = [0] * n     # 1 = index was flipped, 0 = not flipped
        ...
        for i, num in enumerate(nums):
            ...
            if (num == 1 and (currFlipCount & 1) == 0) or (num == 0 and (currFlipCount & 1) == 1):
                continue
            ...
            if (num == 1 and (currFlipCount & 1) == 1) or (num == 0 and (currFlipCount & 1) == 0):
