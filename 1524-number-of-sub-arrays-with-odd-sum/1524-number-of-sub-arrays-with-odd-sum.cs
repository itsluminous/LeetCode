...
public class SolutionN {
    public int NumOfSubarrays(int[] arr) {
        var n = arr.Length;
        var MOD = 1_000_000_007;
        int[] even = new int[n], odd = new int[n];  // count no. of odd & even subarrays till any idx
        var ans = 0;
        ...
        for(var i=1; i<n; i++){
            var isEven = arr[i] % 2 == 0;
            if(isEven){
                odd[i] = odd[i-1];                  // odd + even = odd
                even[i] = (1 + even[i-1]) % MOD;    // even + even = even
            }
            else{
                odd[i] = (1 + even[i-1]) % MOD;     // even + odd = odd
